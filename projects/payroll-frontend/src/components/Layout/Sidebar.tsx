...
const tabs = [
  ...
  { id: 'employees', label: 'Employees', icon: 'M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197m13.5-9a2.25 2.25 0 11-4.5 0 2.25 2.25 0 014.5 0z' },
  { id: 'payroll', label: 'Run Payroll', icon: 'M2.25 18.75a60.07 60.07 0 0115.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75 0 013 6h-.75m0 0v-.375c0-.621.504-1.125 1.125-1.125H20.25M2.25 6v9m18-10.5v.75c0 .414.336.75.75.75h.75m-1.5-1.5h.375c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125h-.375m1.5-1.5H21a.75.75 0 00-.75.75v.75m0 0H3.75m0 0h-.375a1.125 1.125 0 01-1.125-1.125V15m1.5 1.5v-.75A.75.75 0 003 15h-.75M15 10.5a3 3 0 11-6 0 3 3 0 016 0zm3 0h.008v.008H18V10.5zm-12 0h.008v.008H6V10.5z' },
...
const Sidebar = ({ activeTab, onTabChange, isConnected, hasContract, onBack }: SidebarProps) => {
  return (
    <div className="w-64 min-h-screen p-4 border-r" style={{ backgroundColor: '#0A0A0A', borderColor: 'rgba(250,250,247,0.06)' }}>
      {/* Zeril Logo */}
      <div className="flex items-center justify-between mb-8 px-2">
        ...
        <button onClick={onBack} className="text-xs opacity-30 hover:opacity-60 transition-opacity">&larr; Exit</button>
      </div>
        ...
        {tabs.map((tab) => {
          ...
          return (
            <li key={tab.id} className={disabled ? 'disabled opacity-30' : ''}>
              <button
                className={`${activeTab === tab.id ? 'active' : ''} text-sm`}

...
const Sidebar = ({ activeTab, onTabChange, isConnected, hasContract }: SidebarProps) => {
  return (
    <div className="w-64 bg-base-200 min-h-screen p-4">
      <h1 className="text-xl font-bold mb-6 px-2">PayRoll</h1>
      <ul className="menu">
        {tabs.map((tab) => {
          const disabled = !isConnected || (!hasContract && tab.id !== 'settings')
          return (
            <li key={tab.id} className={disabled ? 'disabled opacity-50' : ''}>
              <button
